...
**Configuração:** Tokens definidos via `@theme` no `index.css`.
...
### 1.1 Configuração de Tokens (Tailwind v4)

No Tailwind v4, os tokens devem ser centralizados no `index.css`:
...
### 5.8 Modais (Brutalistas)

Modais devem interromper o fluxo com impacto visual máximo.

- **Overlay:** `bg-slate-950/80 backdrop-blur-none` (evitar blur excessivo).
- **Container:** `border-4 border-slate-950 bg-white shadow-[12px_12px_0px_#000]`.
...
```tsx
<div className="border-4 border-slate-950 mb-4 bg-white shadow-[4px_4px_0px_#000]">
  <button className="w-full p-6 flex justify-between items-center bg-[#ccff00] border-b-4 border-slate-950">
  ...
  <div className="p-6 font-medium">
    Resposta direta e objetiva, mantendo a legibilidade.
  </div>
</div>
```
...
Tooltips são badges pequenos com sombra sólida.

```tsx
<div className="relative group">
  <span className="cursor-help border-b-2 border-dashed border-slate-950">
    Info
  </span>
  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-slate-950 text-[#ccff00] text-[10px] font-black uppercase px-2 py-1 shadow-[4px_4px_0px_#000] whitespace-nowrap">
...
### 6.1 Motion Tokens (Framer Motion)

Para manter a consistência com o Neo-Brutalismo, as animações devem ser "secas" e rápidas.

- **Easing de Impacto:** `[0.23, 1, 0.32, 1]` (Circ out) - para entradas.
- **Easing de Retorno:** `[0.4, 0, 1, 1]` (In) - para saídas.
- **Duração Padrão:** `0.2s` (Micro-interações) | `0.4s` (Transições de seção).
...
## 10. Estados de Feedback e Erro

A consistência na falha é tão importante quanto no sucesso.

### 10.1 Erro (Danger)

- **Cor:** `Red Solid` (`#ff4d4d`)
- **Estética:** Bordas pretas grossas sobre fundo vermelho.
- **Classes:** `bg-[#ff4d4d] text-white border-4 border-slate-950 shadow-[4px_4px_0px_#000]`

### 10.2 Sucesso

- **Cor:** `Green Neon` (`#00ff66`)
- **Classes:** `bg-[#00ff66] text-slate-950 border-4 border-slate-950 shadow-[4px_4px_0px_#000]`

### 10.3 Loading (Skeletons)

- **Estilo:** Blocos sólidos de `slate-200` com animação de pulsação de opacidade.
- **Proibido:** Gradientes "shimmer" suaves. Apenas mudança de opacidade sólida.

```tsx
<div className="w-full h-32 bg-slate-200 animate-pulse border-4 border-slate-950 shadow-[8px_8px_0px_#000]"></div>
```

### 10.4 Acessibilidade (Focus States)

- **Ring:** Sempre use `ring-4 ring-slate-950 ring-offset-2 ring-offset-[#ccff00]`.
- **Contraste:** Garantir que texto sobre Neon Yellow mantenha o padrão `slate-950` para legibilidade.
